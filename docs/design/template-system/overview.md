...
│   └── <templates>
│       ├── 通用基础模板（如 name-spec, table-name-spec）
...
| `content` | 模板内容（Handlebars） | `DROP TABLE {{> table-name-spec}}` |
...
- `{{> table-name-spec}}` - 使用当前上下文
- `{{> table-name-spec ..}}` - 使用父上下文（`..` 表示向上一级）

### 上下文传递

```handlebars
{{!-- 当前上下文：table --}}
{{> table-name-spec}}        {{!-- 使用 table 上下文 --}}
